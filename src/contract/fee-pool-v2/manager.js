...
export class FeePool {
   ...
   */
  async mint(opts={utxo, newNftId: 0, nftRecipient: '', amount: 0, fundingUtxos: [], changeAddress: ''}) {
    if (!opts?.utxo) {
      opts.utxo = await this.getMintingUtxo()
      if (!opts.utxo) throw new Error('No minting utxo found')
    }

    const nextId = opts?.newNftId || Date.now()
    if (isNaN(nextId) || nextId % 1 != 0) throw new Error(`Invalid NFT ID: ${nextId}`)
    ...
    const transaction = contract.functions.mintKeyNft()
      ...
      .to(contract.tokenAddress, 1000n, {
        ...
        amount: opts.utxo?.token?.amount || 0n,
        nft: { capability: "minting", commitment: opts.utxo?.token?.nft?.commitment },
      })
